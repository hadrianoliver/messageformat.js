...
function MessageFormat(locale, pluralFunc, formatters) {
  ...
  this.lc = [];
  locale.every(function eachLocale(loc) {
    pluralFunc ? (this.runtime.pluralFuncs[loc] = pluralFunc) : this._addPluralFuncForLocale(loc);
    for (var l = loc; l; l = l.replace(/[-_]?[^-_]*$/, '')) { this.lc.push(l); }
    return true;
  }, this);
...
MessageFormat.prototype._addPluralFuncForLocale = function(locale) {
  if (typeof locale == 'string') {
    var pf = null, locales = [];
    for (var l = locale; l; l = l.replace(/[-_]?[^-_]*$/, '')) { locales.push(l); pf = MessageFormat.plurals[l]; if (pf) break; }
    if (pf) {
      return this.runtime.pluralFuncs[locale] = pf;
    } else {
      throw new Error('Plural function for locale `' + locales.join(',') + '` not found');
    }
  } else {
    for (var i in locale) { this._addPluralFuncForLocale(locale[i]);  };
  }
}
...
MessageFormat.prototype.compile = function ( messages, opt ) {
  ...
  var pl0 = this.runtime.pluralFuncs;

  if (opt.locale) {
    this.runtime.pluralFuncs = {};
    for (var i in pl0) {
      this.runtime.pluralFuncs[i] = pl0[i];
    }
  }

  for (var ns in messages) {
    if (opt.locale) {
        this.lc = opt.locale[ns] && [].concat(opt.locale[ns]) || lc0;
        this._addPluralFuncForLocale(this.lc);
        if (typeof messages[ns] == 'string') {
          try { r[ns] = compileMsg(self, messages[ns]); }
          catch (e) { e.message = e.message.replace(':', ' with `' + ns + '`' + (baseNs ? ' in `' + baseNs + '`' : '') + ':'); throw e; }
        } else {
