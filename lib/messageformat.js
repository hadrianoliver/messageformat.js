...
/** Find the best-match-plural function for an array of locales or a single locale.
 *  Also stores the locales discovered / passed as this.lc.
 *
 *  @memberof MessageFormat
 *
 *  @param locale - string locale or array of string locales to find plural function for
 *
 *  @private  */
...
MessageFormat.prototype.compile = function ( messages, opt ) {
  ...
  for (var ns in messages) {
    if (opt.locale) {
      if (opt.locale[ns]) {
        var locale = typeof opt.locale[ns] == 'array' ? [].concat(opt.locale[ns]) : opt.locale[ns];
        ...
        if (!this.runtime.pluralFuncs[this.lc[0]]) {
          // Throw error? No plural function was found for the given locale[s].
        }
      } else this.lc = lc0;
    }
